...
export default function MultiFileInputTwo({
  onFilesChange,
}: MultiFileInputProps) {
  ...
  const validateFiles = (newFiles: File[]): File[] => {
    ...
    if (invalidSizeFiles.length > 0) {
      toast.error(
        `Files exceeding 500MB limit: ${invalidSizeFiles.join(', ')}`,
        {
          duration: 4000,
        }
      )
    ...
    <div className='w-full max-w-md mx-auto'>
      ...
      >
        ...
        <p className='text-sm text-gray-400 mt-2'>
          Supported files: Images, Videos, Word, Excel, PDF, ZIP, PPT, PPTX
        </p>
        <p className='text-sm text-gray-400 mt-1'>Maximum file size: 500MB</p>
