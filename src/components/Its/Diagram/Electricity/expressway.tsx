...
import { DiagramElectricityExpressway, useDiagramElecExp } from '@/api/itsAPIs'
...
export default function DiagramElecExpressway() {
  ...
  const checkPermission = checkRole?.children.find((c) => c.name === 'information-diagram')
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      formData.append('utility', updatedProduct.utility)
      formData.append('generatedBy', updatedProduct.generatedBy)
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/diagram/electricity/expressway/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      formData.append('utility', dataList.utility)
      formData.append('location', dataList.location)
      formData.append('generatedBy', dataList.generatedBy)
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/diagram/electricity/expressway/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/diagram/electricity/expressway/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    const initialPayload = {
      ...
      // @ts-ignore
      utility: selectedUtility?.code || '',
      searchQuery: searchKey,
    }

    DiagramElectricityExpressway(initialPayload).then((result) => {
  ...
  const filterSearchForm = (
    <div className='space-y-4'>
      ...
      {/* <div className='space-x-2 w-fit mx-auto'>
        <button
          className='bg-white text-gray-800 border-gray-600 border px-4 py-3 rounded-md font-bold'
          onClick={openNew}
        >
          Upload Document
        </button>
        <button
          className='bg-gray-600 text-white border-gray-600 border font-bold px-4 py-3 rounded-md'
          onClick={exportCSV}
        >
          Download Files
        </button>
  ...
  const { data, isLoading, error, refetch } = useDiagramElecExp(payload)
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/diagram/electricity/expressway/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
          ...
          {updatedProduct && (
            <div>
              <div className='grid grid-cols-2 gap-4'>
                ...
                <div className='field'>
                  <label htmlFor='generatedBy' className='font-bold'>
                    Generated By
                  </label>
                  <InputText
                    id='generatedBy'
                    value={updatedProduct.generatedBy}
                    onChange={(e) =>
                      setUpdatedProduct({
                        ...updatedProduct,
                ...
                {/* <div className='field'>
                  <label htmlFor='utility' className='font-bold'>
                    Select Utility
                  </label>
                  <Dropdown
                    id='utility'
                    value={updatedProduct.utility}
                    options={['Internet', 'Radio', 'LAN', 'CCTV', 'PABX']}
                    onChange={(e) =>
                      setUpdatedProduct({
                        ...updatedProduct,
        ...
        <>
          <div className='grid grid-cols-2 items-center gap-6'>
            ...
            {/* <div className='field'>
              ...
              <Dropdown
                id='utility'
                value={dataList.utility}
                options={['Internet', 'Radio', 'LAN', 'CCTV', 'PABX']}
                onChange={(e) =>
                  setDataList((prev) => ({ ...prev, utility: e.value }))
                }
                placeholder='Select Utility'
              />
            </div> */}
        ...
        {selectedProduct && (
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <p className='break-all'>{selectedProduct.utility}</p>
            </div> */}
              <div>
