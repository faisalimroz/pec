...
import { trainingApi, useTrainingManual } from '@/api/itsAPIs'
...
export default function TrainingData() {
  ...
  const checkPermission = checkRole?.children.find((c) => c.name === 'training')
  ...
  const [filesInput2, setFilesInput2] = useState<File[]>([])
  const [filesInput3, setFilesInput3] = useState<File[]>([])
  ...
  const [newPictureAttachments, setNewPictureAttachments] = useState<File[]>([])
  const [newVideoAttachments, setNewVideoAttachments] = useState<File[]>([])

  const [removedAttachments, setRemovedAttachments] = useState<string[]>([])
  const [removedPictureAttachments, setPictureRemovedAttachments] = useState<
    string[]
  >([])
  const [removedVideoAttachments, setVideoRemovedAttachments] = useState<
    string[]
  >([])
  ...
  const hideUpdateDialog = () => {
    ...
    setNewAttachments([])
    setNewPictureAttachments([])
    setNewVideoAttachments([])

    setRemovedAttachments([])
    setPictureRemovedAttachments([])
    setVideoRemovedAttachments([])
  }
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      formData.append('trainer', updatedProduct.trainer)
      ...
      newPictureAttachments.forEach((file) => {
        formData.append('pictures', file)
      })

      newVideoAttachments.forEach((file) => {
      ...
      removedPictureAttachments.forEach((attachmentId) => {
        formData.append('removedPictures', attachmentId)
      })

      removedVideoAttachments.forEach((attachmentId) => {
        formData.append('removedVideos', attachmentId)
      })

      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/traning/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const handleNewPictureAttachments = (files: File[]) => {
    setNewPictureAttachments(files)
  }

  const handleNewVideoAttachments = (files: File[]) => {
    setNewVideoAttachments(files)
  }
  ...
  const handleRemoveAttachment = (attachmentId: string) => {
    ...
    setUpdatedProduct((prev) => {
      ...
      return {
        ...prev,
        attachment: prev.attachment.filter((a) => a._id !== attachmentId),
      }
    })
  }

  const handleRemovePictureAttachment = (attachmentId: string) => {
    setPictureRemovedAttachments((prev) => [...prev, attachmentId])
    setUpdatedProduct((prev) => {
      if (!prev) return null
      return {
        ...prev,
        pictures: prev.pictures.filter((a) => a._id !== attachmentId),
      }
    })
  }

  const handleRemoveVideoAttachment = (attachmentId: string) => {
    setVideoRemovedAttachments((prev) => [...prev, attachmentId])
    setUpdatedProduct((prev) => {
      if (!prev) return null
      return {
        ...prev,
        video: prev.video.filter((a) => a._id !== attachmentId),
      }
    })
  }
  ...
  const saveProduct = async () => {
    try {
      ...
      formData.append('location', dataList.location)
      formData.append('trainer', dataList.trainer)
      formData.append('remarks', remarks)
      formData.append('date', formatDate(formDate))
      filesInput.forEach((file) => {
        formData.append('attachment', file)
      })
      filesInput2.forEach((file) => {
        formData.append('pictures', file)
      })
      filesInput3.forEach((file) => {
        formData.append('video', file)
      })

      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/traning/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/traning/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/traning/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
    ...
    if (isITS) {
      items.push(
        ...
        },
        {
          ...
          icon: 'pi pi-download',
          command: () => downloadAttachmentsAsZip(rowData.attachment),
        }
      )
    }
  ...
  const videoBodyTemplate = (rowData: any) => {
    return <div>{rowData?.video?.length}</div>
  }
  ...
  const { data, isLoading, error, refetch } = useTrainingManual(payload)
  ...
  const handlePictureChange = (newFiles: File[]) => {
    setFilesInput2(newFiles)
  }

  const handleVideoChange = (newFiles: File[]) => {
    setFilesInput3(newFiles)
  }
    ...
    <div className='ml-4'>
      ...
      >
        <>
          <div className='grid grid-cols-2 items-center gap-6'>
            ...
            <div className='field'>
              <label htmlFor='trainer' className='font-bold'>
                Trainer
              </label>
              <InputText
                id='trainer'
                value={dataList.trainer}
                onChange={handleInputChange}
              />
            </div>
          ...
          <div className='gap-3 mt-5'>
            ...
            <div>
              <MultiFileInput onFilesChange={handlePictureChange} />
            </div>
          </div>
          ...
          <div className='gap-3 mt-5'>
            ...
            <div>
              <MultiFileInput onFilesChange={handleVideoChange} />
            </div>
          </div>
        </>
      </Dialog>
      ...
      >
        <div className='confirmation-content'>
          ...
          {product && (
            <span>
              Are you sure you want to delete <b>{product.subject}</b>?
            </span>
          )}
        </div>
      </Dialog>
          ...
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                ...
                <p className='break-all'>{selectedProduct.trainer}</p>
              </div>
              <div>
              ...
              {isITS && (
                <>
                  <div className='col-span-2'>
                    ...
                    <div className='grid grid-cols-2 gap-4'>
                      {selectedProduct.attachment.map((attachment) => (
                        <div
                          key={attachment._id}
                  ...
                  <div className='col-span-2'>
                    <h3 className='font-bold'>Picture Attachments</h3>
                    <div className='w-fit mt-2 flex flex-wrap gap-6 justify-start'>
                      {selectedProduct.pictures.map((attachment, index) => (
                        <div>
                          <FilePreview url={attachment.url} />
                          <Button
                            key={attachment._id}
                  ...
                  <div className='col-span-2'>
                    <h3 className='font-bold'>Video Attachments</h3>
                    <div className='grid grid-cols-2 gap-4'>
                      {selectedProduct.video.map((attachment) => (
                        <div
                          key={attachment._id}
                          className='border rounded-lg p-3 hover:shadow-md transition-shadow cursor-pointer'
                        >
          ...
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='col-span-2'>
              ...
              <div className='flex flex-wrap gap-3'>
                {updatedProduct.attachment.map((attachment) => (
                  <div
                    key={attachment._id}
            ...
            <div className='col-span-2'>
              ...
              <h3 className='font-bold mb-2'>Existing Picture Attachments</h3>
              <div className='flex flex-wrap gap-3'>
                {updatedProduct.pictures.map((pictures) => (
                  <div
                    key={pictures._id}
                    className='flex items-center gap-2 bg-gray-100 p-1 rounded-md'
                  >
                    <a
                      ...
                      className='text-blue-600 hover:underline'
                    >
                      {pictures.url?.split('/').pop()}
                    </a>
                    <Button
                      icon='pi pi-times text-red-500'
                      className='p-button-rounded text-sm text-red-500 ml-2'
                      onClick={() =>
                        handleRemovePictureAttachment(pictures._id)
                      }
                    />
                  </div>
                ))}
            ...
            <div className='col-span-2'>
              <h3 className='font-bold mb-2'>Add New Pictures Attachments</h3>
              <MultiFileInput onFilesChange={handleNewPictureAttachments} />
            </div>
            ...
            <div className='col-span-2'>
              ...
              <h3 className='font-bold mb-2'>Existing Video Attachments</h3>
              <div className='flex flex-wrap gap-3'>
                {updatedProduct.video.map((video) => (
                  <div
                    key={video._id}
                    className='flex items-center gap-2 bg-gray-100 p-1 rounded-md'
                  >
                    ...
                    <Button
                      icon='pi pi-times text-red-500'
                      className='p-button-rounded text-sm text-red-500 ml-2'
                      onClick={() => handleRemoveVideoAttachment(video._id)}
                    />
                  </div>
                ))}
              </div>
            ...
            <div className='col-span-2'>
              <h3 className='font-bold mb-2'>Add New Attachments</h3>
              <MultiFileInput onFilesChange={handleNewVideoAttachments} />
            </div>
          </div>
        )}
      </Dialog>
