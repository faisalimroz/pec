...
import { InventoryElectricityTool, useInventoryTools } from '@/api/itsAPIs'
...
export default function InventoryElecTool() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/tool/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/tool/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/tool/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    InventoryElectricityTool(initialPayload).then((result) => {
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/tool/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const { data, isLoading, error, refetch } = useInventoryTools(payload)
