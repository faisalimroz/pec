...
import { InventoryElectricityEquip, useInventoryEquip } from '@/api/itsAPIs'
...
export default function InventoryElecEquipment() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/equipment/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/equipment/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/equipment/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    InventoryElectricityEquip(initialPayload).then((result) => {
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/equipment/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
      ...
      <>
        {isITS && (
          <div className='space-x-2'>
            ...
            <button
              className='bg-gray-600 text-white border-gray-600 border-t border-l border-r font-bold px-4 py-3 rounded-t-md texts-m'
              onClick={exportCSV}
            >
              Download Files{' '}
  ...
  const { data, isLoading, error, refetch } = useInventoryEquip(payload)
