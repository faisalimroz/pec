...
import {
  InventoryElectricityStationary,
  useInventoryStationary,
} from '@/api/itsAPIs'
...
export default function InventoryElecStationary() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/stationary/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/stationary/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/inventory/electricity/stationary/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    InventoryElectricityStationary(initialPayload).then((result) => {
  ...
  const { data, isLoading, error, refetch } = useInventoryStationary(payload)
  ...
  return (
    <>
      <Toolbar
        className='rounded-none border-none p-0 bg-white m-0'
