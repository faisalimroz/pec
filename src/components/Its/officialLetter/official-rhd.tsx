...
import { searchOfficialRhdData, useRhd } from '@/api/itsAPIs'
...
export default function OfficialRhd() {
  ...
  const checkPermission = checkRole?.children.find(
    (c) => c.name === 'internal-letter-announce'
  )
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/official-letter/rhd/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/official-letter/rhd/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/official-letter/rhd/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/official-letter/rhd/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchOfficialRhdData(initialPayload).then((result) => {
  ...
  const { data, isLoading, error, refetch } = useRhd(payload)
        ...
        {selectedProduct && (
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                <h3 className='font-bold'>From/To</h3>
                <p className='break-all'>
                  {selectedProduct.status} {selectedProduct?.name}
                </p>
              </div>
              <div>
