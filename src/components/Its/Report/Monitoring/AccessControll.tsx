...
import { MonitoringAccessControll, useReportAC } from '@/api/itsAPIs'
...
export default function AccessControll() {
  ...
  const checkPermission = checkRole?.children.find((c) => c.name === 'report')
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/access-control/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/access-control/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/access-control/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    const initialPayload = {
      ...
      // @ts-ignore
      site: selectedPlace?.code || '',
      searchQuery: searchKey,
    }

    MonitoringAccessControll(initialPayload).then((result) => {
  ...
  const { data, isLoading, error, refetch } = useReportAC(payload)
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/access-control/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
