...
import { MonitoringInternet, useReportNetwork } from '@/api/itsAPIs'
...
export default function InternetStatus() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/internet/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/internet/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/internet/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    const initialPayload = {
      ...
      // @ts-ignore
      types: selectedPlace?.code || '',
      searchQuery: searchKey,
    }

    MonitoringInternet(initialPayload).then((result) => {
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/report/monitoring/internet/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
      ...
      <>
        {isITS && (
          <div className='space-x-2'>
            <button
              className='bg-white text-gray-800 border-gray-600 text-sm border-t border-l border-r px-4 py-3 rounded-t-md font-bold'
              onClick={openNew}
            >
              Upload Document
            </button>
            <button
              className='bg-gray-600 text-white border-gray-600 text-sm border-t border-l border-r font-bold px-4 py-3 rounded-t-md'
              onClick={exportCSV}
            >
              Download Files{' '}
  ...
  const { data, isLoading, error, refetch } = useReportNetwork(payload)
          ...
          <div className='grid grid-cols-2 items-center gap-6'>
            ...
            <div className='field'>
              ...
              <Dropdown
                id='type'
                value={type}
                options={['Internet', 'LAN', 'CCTV', 'PABX', 'Miscellaneous']}
