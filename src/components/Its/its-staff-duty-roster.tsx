...
import { searchItsDutyRoster, useSearchItsDutyRoster } from '@/api/itsAPIs'
...
export default function staffDutyRoster() {
  ...
  const {
    data: dutyRosterData,
    isLoading: dutyRosterLoading,
    error: dutyRosterError,
    refetch,
  } = useSearchItsDutyRoster(payload)
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/duty-roster/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/duty-roster/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/duty-roster/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/its/duty-roster/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchItsDutyRoster(initialPayload).then((result) => {
  ...
  const refetchData = () => {
    if (dutyRosterData) {
      setProducts(dutyRosterData?.MoneysByMonths)
    }
  }
