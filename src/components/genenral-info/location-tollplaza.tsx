...
export default function TollPlazaLocation() {
  ...
  const fetchProjects = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/v1/general/tollplaza_location/all`,
        {
          headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/general/tollplaza_location/update/${selectedProject._id}`,
        formData,
        {
          headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/general/tollplaza_location/delete/${selectedProject._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/general/tollplaza_location/create/layout`,
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
                  ...
                  <div className='mt-1'>
                    ...
                    {selectedProject?.creationTimestamp && (
                      <p className='text-sm text-gray-600'>
                        <span>
                          Date:{' '}
                          {selectedProject.creationTimestamp.split(' ')[0]}
                        </span>
                        <span className='mx-1'>•</span>
                        <span>
                          Time:{' '}
                          {selectedProject.creationTimestamp.split(' ')[1]}
                        </span>
                      </p>
                    )}
                  </div>
                  ...
                  <div className='mt-1'>
                    ...
                    {selectedProject?.updatingTimestamp && (
                      <p className='text-sm text-gray-600'>
                        <span>
                          Date:{' '}
                          {selectedProject.updatingTimestamp.split(' ')[0]}
                        </span>
                        <span className='mx-1'>•</span>
                        <span>
                          Time:{' '}
                          {selectedProject.updatingTimestamp.split(' ')[1]}
                        </span>
                      </p>
                    )}
                  </div>
