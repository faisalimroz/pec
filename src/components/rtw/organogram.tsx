...
const OrgChart: React.FC = () => {
  ...
  // Permissions Logic
  const checkRole = permissions.find((p) => p.name === 'rtw-manager')
  const checkPermission = checkRole?.children.find(
    (c) => c.name === 'rtw-organom')
  ...
  const isGeneral = rtwPermission?.edit_authority === true;
  ...
  const fetchLayout = async () => {
    ...
    try {
      const response = await axios.get<ApiResponse>(
        `${import.meta.env.VITE_BASE_URL}/api/v1/rtw/organization/organization-chart`,
        {
          headers: {
