...
import { searchBMWorkCompletion } from '@/api/roadTrafficAPIs'
...
export default function BMWorkCompletionTable() {
  ...
  const checkPermission = checkRole?.children.find(
    (c) => c.name === 'building-maint'
  )

  const isRnT = checkPermission?.edit_authority || false

  const codes = [
    { name: 'Daily Work Report', code: 'daily-report' },
    { name: 'Inspection Report', code: 'inspection-report' },
    { name: 'Work Completion Report', code: 'work-report' },
  ]
  ...
  const [workCompilition, setWorkCompilition] = useState('')
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      formData.append('workCompilition', updatedProduct.workCompilition)
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/building-maintenance/work-completion/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/building-maintenance/work-completion/bulk-upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const saveProduct = async () => {
    try {
      ...
      formData.append('workCompilition', workCompilition)
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/building-maintenance/work-completion/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/building-maintenance/work-completion/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/building-maintenance/work-completion/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchBMWorkCompletion(initialPayload).then((result) => {
      setProducts(result?.BuildingMaintenances)
      setLoading(false)
    })
  }
          ...
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                ...
                <p className='break-all'>{selectedProduct.workCompilition}</p>
              </div>
              <div>
      ...
      >
        <>
          <div className='grid grid-cols-2 items-center gap-6'>
            ...
            <div className='field'>
              <label htmlFor='workCompilition' className='font-bold'>
                Work Completion
              </label>
              <InputText
                id='workCompilition'
                onChange={(e) => setWorkCompilition(e.target.value)}
                required
              />
            </div>
        ...
        <div className='flex flex-col mx-auto text-center space-y-2'>
          ...
          {product && (
            <span className='text-red-500'>
              Are you sure you want to delete <b>{product.docTitle}</b>?
            </span>
          )}
        </div>
      </Dialog>
        ...
        {updatedProduct && (
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='field'>
              ...
              <InputText
                id='productModel'
                value={updatedProduct.workCompilition}
                onChange={(e) =>
                  setUpdatedProduct({
                    ...updatedProduct,
                    workCompilition: e.target.value,
                  })
                }
                required
              />
