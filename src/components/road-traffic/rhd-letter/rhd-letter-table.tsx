...
import { searchRhdLetter } from '@/api/roadTrafficAPIs'
...
export default function RhdLetterTable() {
  ...
  const codes = [
    { name: 'Client Letter', code: 'Client Letter' },
    { name: 'KEC Letter', code: 'KEC Letter' },
  ]
...
const [approved, setApproved] = useState<boolean>(false);
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      formData.append('referenceNo', updatedProduct.referenceNo)
      formData.append('type', updatedProduct.type)
      formData.append('remarks', updatedProduct.remarks)
      formData.append('issueDate', updatedProduct.issueDate)
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/rhd-letter/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      formData.append('issueDate', formatDate(formDate))
      filesInput.forEach((file) => {
        formData.append('attachments', file)
      })

      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/rhd-letter/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/rhd-letter/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchRhdLetter(initialPayload).then((result) => {
      setProducts(result?.RhdLetters)
      setLoading(false)
    })
  }
        ...
        {selectedProduct && (
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div>
              ...
              <p className='break-all'>{selectedProduct.issueDate}</p>
            </div>
            <div>
            ...
            <div>
              <h3 className='font-bold'>Reference No</h3>
              <p className='break-all'>{selectedProduct.referenceNo}</p>
            </div>
            <div>
      ...
      >
        <>
          <div className='grid grid-cols-2 items-center gap-6'>
            ...
            <div className='field'>
              <label htmlFor='referenceNo' className='font-bold'>
                Reference No.
              </label>
              <InputText
                id='referenceNo'
            ...
            <div className='field'>
              ...
              <Dropdown
                id='type'
                value={type}
                options={['Client Letter', 'KEC Letter']}
        ...
        {updatedProduct && (
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='field'>
              ...
              <InputText
                id='referenceNo'
                value={updatedProduct.referenceNo}
                onChange={(e) =>
                  setUpdatedProduct({
                    ...updatedProduct,
            ...
            <div className='field'>
              ...
              <Calendar
                ...
                value={
                  new Date(
                    updatedProduct.issueDate.split('-').reverse().join('-')
                  )
                }
                onChange={(e) =>
                  setUpdatedProduct({
                    ...updatedProduct,
                    issueDate: e.value ? formatDate(e.value) : '',
                  })
                }
                dateFormat='dd/mm/yy'
              />
