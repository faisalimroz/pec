...
import { searchPoliceRecord } from '@/api/roadTrafficAPIs'
...
export default function PoliceRecordTable() {
  ...
  const codes = [
    ...
    { name: 'CH:25+501 TO CH:27+000', value: 'CH:25+501 TO CH:27+000' }, // Duplicate removed
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      formData.append('incident', updatedProduct.incident)
      formData.append('location', updatedProduct.location)
      formData.append('remarks', updatedProduct.remarks)
      // formData.append('zone', updatedProduct.zone)
      formData.append('time', updatedProduct.time)
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/roadsafety/police_record/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
            'Content-Zone': 'multipart/form-data',
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/roadsafety/police_record/bulk_upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const saveProduct = async () => {
    try {
      ...
      if (time) {
        formData.append('time', convertToTimeOnly(time))
      }
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/roadsafety/police_record/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/roadsafety/police_record/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/roadsafety/police_record/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchPoliceRecord(initialPayload).then((result) => {
      setProducts(result?.PoliceRecords)
      setLoading(false)
    })
  }
        ...
        {selectedProduct && (
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <p className='break-all'>{selectedProduct.zone}</p>
            </div> */}
              <div>
                <h3 className='font-bold'>Incident</h3>
                <p className='break-all'>{selectedProduct.incident}</p>
              </div>
              <div>
                <h3 className='font-bold'>FIR/GD</h3>
