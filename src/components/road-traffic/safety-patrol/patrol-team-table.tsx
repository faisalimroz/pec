...
import { searchPatrolTeam } from '@/api/roadTrafficAPIs'
...
export default function PatrolTeamTable() {
  ...
  const codes = [
    { name: 'Patrol Team Report', code: 'patrol-team-report' },
    { name: 'Rectification Report', code: 'rectification-report' },
    { name: 'Police Data Report', code: 'police-data-report' },
  ]
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/safetypatrol/patrolteam/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/safetypatrol/patrolteam/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/safetypatrol/patrolteam/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchPatrolTeam(initialPayload).then((result) => {
      setProducts(result?.PatrolTeams)
      setLoading(false)
    })
  }
