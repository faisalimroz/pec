...
import { searchSecurityTeam } from '@/api/roadTrafficAPIs'
...
export default function SecurityTeamTable() {
  ...
  const security_codes = [
    { name: 'Flyover Man', code: 'patrol-team-report' },
    { name: 'Night Security', code: 'rectification-report' },
    { name: 'Ansar & Teletel', code: 'police-data-report' },
  ]
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/safetypatrol/securityteam/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/safetypatrol/securityteam/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/road-traffic/safetypatrol/securityteam/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchSecurityTeam(initialPayload).then((result) => {
      setProducts(result?.SecurityTeams)
      setLoading(false)
    })
  }
          ...
          <div className='grid grid-cols-2 items-center gap-6'>
            ...
            <div className='field'>
              ...
              <Dropdown
                id='type'
                value={type}
                options={['Flyover Man', 'Night Security', 'Ansar & Teletel']}
