import { searchTollTrafficVer } from '@/api/tollApi'
...
export default function TollTrafficVerTable() {
  ...
  const [totalTypeToll, setTotalTypeToll] = useState('')
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/special/others/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
      ...
      searchTollTrafficVer(initialPayload).then((result) => {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/special/others/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/special/others/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchTollTrafficVer(initialPayload).then((result) => {
      setProducts(result?.data)
      setTotalPass(result?.allTotalPass)
      setTotalAmount(result?.allTotalAmount)
      setTotalTypeToll(result?.allTypeTollTotalpass)
      setLoading(false)
    })
  }
  ...
  const footerGroup = (
    <ColumnGroup>
      <Row>
        ...
        />
        <Column footer={totalPass} />
        <Column footer={totalTypeToll} />
      ...
      <div className='card'>
        ...
        >
          ...
          <Column
            ...
            bodyClassName='text-xs truncate max-w-xs'
            sortable
            header='Total Pass(With Exemption)'
          ></Column>

          <Column
            field='typeTollTotalpass'
            headerClassName='bg-[#ffc2c2] text-sm'
            bodyClassName='text-xs truncate max-w-xs'
            sortable
            header='Total Pass(Without Exemption)'
          ></Column>
