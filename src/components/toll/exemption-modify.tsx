import { searchExemptionModify } from '@/api/tollApi'
...
export default function ExemptionModifyTable() {
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/special/exemption/vehicle/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchExemptionModify(initialPayload).then((result) => {
      setProducts(result)
      setLoading(false)
    })
  }
