import { searchMonthlyRoster, useMonthlyRoster } from '@/api/tollApi'
...
export default function ReceivedTable() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/monthlyroaster/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
            'Content-Month': 'multipart/form-data',
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/monthlyroaster/bulk-upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/monthlyroaster/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/monthlyroaster/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/monthlyroaster/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchMonthlyRoster(initialPayload).then((result) => {
  ...
  const { data, isLoading, error, refetch } = useMonthlyRoster(payload)

  // initial data load
  useEffect(() => {
    if (data) {
      setProducts(data?.MonthlyRoasters)
    }
  }, [data])
          ...
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                ...
                <p className='break-all'>{selectedProduct.remark}</p>
              </div>
