import { searchEtcCardModify } from '@/api/tollApi'
...
export default function EtcCardModifyTable() {
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/collection/etc/card/vehicle/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/toll/collection/etc/card/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    const initialPayload = {
      location: selectedUtility ? selectedUtility : '',
      searchQuery: searchKey,
    }

    searchEtcCardModify(initialPayload).then((result) => {
      setProducts(result)
      setLoading(false)
    })
  }
