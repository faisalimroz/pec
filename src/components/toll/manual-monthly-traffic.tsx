...
import {
  searchManualMonthlyTrafficR,
  useManualMonthlyTrafficR,
} from '@/api/tollApi'
...
export default function ManualMonthlyTrafficTable() {
  ...
  const handleSearch = () => {
    ...
    searchManualMonthlyTrafficR(initialPayload).then((result) => {
  ...
  const { data, isLoading, error, refetch } = useManualMonthlyTrafficR(payload)
