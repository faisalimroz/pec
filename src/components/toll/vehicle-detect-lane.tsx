...
import { searchVehicleDetectLane } from '@/api/tollApi'
...
export default function VehicleDetectLaneTable({ id }: Props) {
  ...
  const actionBodyTemplate = (rowData: any) => {
    ...
    return (
      <>
        <Link
          to={`/toll/vehicle-detect-and-toll/vehicle-type/${rowData?.vehicle_types}?lane=${id}`}
  ...
  const handleReset = () => {
    ...
    searchVehicleDetectLane(initialPayload).then((result) => {
      setProducts(result)
      setLoading(false)
    })
  }
  ...
  const vehicleBodyTemp = (rowData: any) => {
    return (
      <div className='flex flex-col justify-center items-center'>
        <img
          // @ts-ignore
          src={vehicleIcons[rowData.vehicle_types]}
          alt={rowData.vehicle_types}
          className='mb-2'
        />
        <h1 className='text-center font-semibold'>
          {formatVehicleName(rowData.vehicle_types)}
        </h1>
      </div>
    )
  }
  ...
  return (
    <div className='ml-4'>
      <Link
        to='/toll/vehicle-detect-and-toll'
