import { searchMaintainIpcPdf, useMaintainIpcPdf } from '@/api/financeAPIs'
...
export default function MaintainIpcPdfTable() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/ipc-maintain/pdf/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/ipc-maintain/pdf/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/ipc-maintain/pdf/bulk_upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/ipc-maintain/pdf/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/ipc-maintain/pdf/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchMaintainIpcPdf(initialPayload).then((result) => {
      setProducts(result?.MaintainIpcs)
      setLoading(false)
    })
  }
  ...
  const { data, isLoading, error, refetch } = useMaintainIpcPdf(payload)

  // initial data load
  useEffect(() => {
    if (data) {
      setProducts(data?.MaintainIpcs)
    }
  }, [data])
            ...
            <div className='field'>
              ...
              <InputText
                ...
                className={classNames({
                  'p-invalid': submitted && !ipcNo,
                })}
              />
              {submitted && !ipcNo && (
                <small className='p-error'>Ipc No is required.</small>
              )}
            </div>
