import { searchMonthlySalary, useMonthlySalarySheet } from '@/api/financeAPIs'
...
export default function MonthlySalarySheet() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/salary/monthly-sheet/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/salary/monthly-sheet/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/salary/monthly-sheet/bulk_upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/salary/monthly-sheet/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchMonthlySalary(initialPayload).then((result) => {
  ...
  const { data, isLoading, error, refetch } = useMonthlySalarySheet(payload)

  // initial data load
  useEffect(() => {
    if (data) {
      setProducts(data?.MonthlySalarys)
    }
  }, [data])
        ...
        {selectedProduct && (
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div className='space-y-2'>
                <h3 className='font-bold flex'>File Name</h3>
