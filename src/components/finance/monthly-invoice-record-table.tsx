...
import {
  searchMonthlyInvoiceExl,
  searchMonthlyInvoicePdf,
  useMonthlyInvoiceRecord,
...
export default function RhdBillDetails() {
  ...
  const checkPermission = checkRole?.children.find(
    (c) => c.name === 'monthly-invoice-record'
  )
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/invoice/excel/monthly/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/invoice/excel/monthly/bulk_upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/invoice/excel/monthly/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/invoice/excel/monthly`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/invoice/excel/monthly/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchMonthlyInvoiceExl(initialPayload).then((result) => {
  ...
  const {
    ...
    error,
    refetch: refetchExl,
  } = useMonthlyInvoiceRecord(payload)
