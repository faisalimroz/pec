import { searchMonthlyPit, useMonthlyPitSheet } from '@/api/financeAPIs'
...
export default function MonthlyPitSheetTable() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/pit/monthly-sheet/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/pit/monthly-sheet/bulk_upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/pit/monthly-sheet/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/pit/monthly-sheet/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/pit/monthly-sheet/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchMonthlyPit(initialPayload).then((result) => {
      setProducts(result?.MonthlyPits)
      setLoading(false)
    })
  }
  ...
  const { data, isLoading, error, refetch } = useMonthlyPitSheet(payload)

  // initial data load
  useEffect(() => {
    if (data) {
      setProducts(data?.MonthlyPits)
    }
  }, [data])
