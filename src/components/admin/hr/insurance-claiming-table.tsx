...
import { searchInsuranceClaiming } from '@/api/adminAPIs'
...
export default function MedicineRecordTable() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/insurance-claiming/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      formData.append('claimingDate', formatDate(formDate))
      filesInput.forEach((file) => {
        formData.append('attachments', file)
      })

      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/insurance-claiming/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/insurance-claiming/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/insurance-claiming/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchInsuranceClaiming(initialPayload).then((result) => {
      setProducts(result?.Labors)
      setLoading(false)
    })
  }
      ...
      <div className='card'>
        ...
        >
          ...
          <Column
            ...
            bodyClassName='text-sm truncate max-w-xs'
            // sortable
            header='Insurance Claiming Date'
          ></Column>
      ...
      >
        {updatedProduct && (
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='field'>
              <label htmlFor='claimingDate' className='font-bold'>
                Insurance Claiming Date
              </label>
              <Calendar
                id='claimingDate'
                value={
                  new Date(
                    updatedProduct.claimingDate.split('-').reverse().join('-')
                  )
                }
                onChange={(e) =>
                  setUpdatedProduct({
                    ...updatedProduct,
                    claimingDate: e.value ? formatDate(e.value) : '',
                  })
                }
                dateFormat='dd/mm/yy'
              />
      ...
      >
        {selectedProduct && (
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div>
                <h3 className='font-bold'>ID No.</h3>
                <p className='break-all'>{selectedProduct.cId}</p>
              </div>
              <div>
                <h3 className='font-bold'>Insurance Claiming Date</h3>
                <p>{selectedProduct.claimingDate}</p>
              </div>
              <div>
        ...
        <>
          <div className='grid grid-cols-2 items-center gap-6'>
            <div className='field'>
              ...
              <InputText id='cId' onChange={(e) => setCId(e.target.value)} />
            </div>
            ...
            <div className='field'>
              ...
              <InputText id='dept' onChange={(e) => setDept(e.target.value)} />
            </div>
