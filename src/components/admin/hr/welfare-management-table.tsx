...
import { searchWelfareManagement } from '@/api/adminAPIs'
...
export default function WelfareManagementTable() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/welfare/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/welfare/upload`,
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/welfare/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchWelfareManagement(initialPayload).then((result) => {
      setProducts(result?.Welfares)
      setLoading(false)
    })
  }
