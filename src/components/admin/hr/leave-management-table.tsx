...
import { searchLeaveManagement } from '@/api/adminAPIs'
...
export default function LeaveManagementTable() {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      formData.append('reason', updatedProduct.reason)
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/leave/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/leave/upload`,
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/leave/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/hr/leave/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchLeaveManagement(initialPayload).then((result) => {
      setProducts(result?.Leaves)
      setLoading(false)
    })
  }
          ...
          <>
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div>
                ...
                <p className='break-all'>{selectedProduct.reason}</p>
              </div>
              <div>
