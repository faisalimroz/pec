...
export default function InsuranceManageDetailNew({
  id,
  isDialog = false,
  ...
  const [removedInsuranceClaiming, setRemovedInsuranceClaiming] = useState<string[]>([])
  ...
  const handleUpdateEmployee = async () => {
    ...
    try {
      ...
      newInsuranceClaimings.forEach((file) => formData.append('insuranceClaiming', file))
  ...
  const downloadEmployeePDF = async () => {
    try {
      ...
      if (documents.some((doc) => doc.files && doc.files.length > 0)) {
        ...
        if (yPos > pageHeight - 40) { // 40 is an arbitrary value for section space
          pdf.addPage()
          yPos = margin
        }
  ...
  const documents = [
    ...
    // { label: 'Dismissal or Termination', files: employeeData.termination },
    {
      label:
        employeeData.insuranceClaiming.length > 0
          ? 'Insurance Claiming'
          : 'No Insurance Claiming',
      files: employeeData.insuranceClaiming,
    },
  ]
      ...
      >
        {updatedEmployee && (
          <>
            ...
            <div className='field mb-2'>
              <label htmlFor='newInsuranceClaimings' className='block font-bold mb-2'>
