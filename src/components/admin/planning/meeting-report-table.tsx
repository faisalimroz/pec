...
import { searchMeetingReport } from '@/api/adminAPIs'
...
export default function MeetingReportTable() {
  ...
  const [minOfmeeting, setMinutes] = useState('')
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      formData.append('minOfmeeting', updatedProduct.minOfmeeting)
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/plan/meeting/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const saveProduct = async () => {
    try {
      ...
      formData.append('minOfmeeting', minOfmeeting)
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/plan/meeting/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/admin/plan/meeting/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchMeetingReport(initialPayload).then((result) => {
      setProducts(result?.PlaningMeetings)
      setLoading(false)
    })
  }
      ...
      >
        {updatedProduct && (
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='field'>
              <label htmlFor='minOfmeeting' className='font-bold'>
                Minutes Of Meeting
              </label>
              <InputText
                id='minOfmeeting'
                value={updatedProduct.minOfmeeting}
                onChange={(e) =>
                  setUpdatedProduct({
                    ...updatedProduct,
                    minOfmeeting: e.target.value,
                  })
                }
              />
            </div>
      ...
      >
        {selectedProduct && (
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div>
              <h3 className='font-bold'>Minutes Of Meening</h3>
              <p className='break-all'>{selectedProduct.minOfmeeting}</p>
            </div>
            <div>
