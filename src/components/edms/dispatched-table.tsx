import { searchDispatched, useDispatched } from '@/api/edmsAPIs'
...
export default function DispatchedTable() {
  ...
  const hasEditAccess = dispatchedPermission?.edit_authority || false
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/edms/dispatched/update/${updatedProduct._id}`,
        formData,
        {
          headers: {
  ...
  const uploadFile = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/edms/dispatched/bulk-upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const saveProduct = async () => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/v1/edms/dispatched/upload`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/edms/dispatched/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/edms/dispatched/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchDispatched(initialPayload).then((result) => {
      setProducts(result?.DispatchedEDMSs)
      setLoading(false)
    })
  }
  ...
  const { data, isLoading, error, refetch } = useDispatched(payload)

  // initial data load
  useEffect(() => {
    if (data) {
      setProducts(data?.DispatchedEDMSs)
    }
  }, [data])
            ...
            <div className='grid grid-cols-2 gap-4'>
              ...
              {hasEditAccess && (
                <div className='col-span-2'>
                  ...
                  <div className='w-fit mt-2 flex flex-col justify-start gap-2'>
                    {selectedProduct.attachments.map(
                      (attachment: any, index) => (
                        <button
                          key={attachment._id}
                          onClick={() => handleDownload(attachment.url, index)}
                          disabled={downloadingIndex === index}
                          className={`flex items-center gap-2 border px-4 py-2 rounded-md text-sm font-medium transition ${
                            downloadingIndex === index
                              ? 'opacity-50 cursor-not-allowed'
                              : 'hover:text-blue-600/70'
                          }`}
                        >
                          <i className='pi pi-download' />
                          {downloadingIndex === index
                            ? 'Downloading...'
                            : `File No. ${index + 1}: ${decodeURIComponent(
