...
import { searchUserLog, useSearchUserLog } from '@/api/noticeApis'
...
export default function UserLog() {
  ...
  const { roles } = useAuth()
  const isSuperAdmin = roles.some((role) => ['superadmin'].includes(role.title))
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/logs/delete/${product._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const handleSearch = () => {
    ...
    searchUserLog(initialPayload).then((result) => {
      setProducts(result?.logs)
      setLoading(false)
    })
  }
  ...
  const deleteSelectedProducts = async () => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/v1/logs/delete/multiple/data`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const { data, isLoading, error, refetch } = useSearchUserLog(payload)
